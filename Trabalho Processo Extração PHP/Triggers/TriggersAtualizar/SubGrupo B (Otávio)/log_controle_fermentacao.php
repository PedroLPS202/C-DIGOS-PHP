...
function CriarTriggers($conn) {
    ...
    $sqlCriandoTriggers = "CREATE TRIGGER IF NOT EXISTS controle_fermentacao_after_update
        AFTER UPDATE ON controle_fermentacao
        FOR EACH ROW
        BEGIN
            INSERT INTO log_controle_fermentacao (id, alteracao) VALUES (NEW.id, 'atualização');
        END;
    ";
