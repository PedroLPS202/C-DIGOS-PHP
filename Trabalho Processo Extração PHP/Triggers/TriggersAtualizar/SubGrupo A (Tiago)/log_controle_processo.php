...
function CriarTriggers($conn) {
    // Triggers para a tabela controle_processo
    $sqlCriandoTriggers = "CREATE TRIGGER IF NOT EXISTS controle_processo_after_update
        AFTER UPDATE ON controle_processo
        FOR EACH ROW
        BEGIN
            INSERT INTO log_controle_processo (id, alteracao) VALUES (NEW.id, 'atualização');
        END;
    ";
