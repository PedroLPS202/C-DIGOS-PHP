...
function CriarTriggers($conn) {
    // Triggers para a tabela moagem_trituracao
    $sqlCriandoTriggers = "CREATE TRIGGER IF NOT EXISTS moagem_trituracao_after_update
        AFTER UPDATE ON moagem_trituracao
        FOR EACH ROW
        BEGIN
            INSERT INTO  log_moagem_trituracao (id, alteracao) VALUES (NEW.id, 'atualização');
        END;
    ";
