...
function CriarTriggers($conn) {
    ...
    $sqlCriandoTriggers = "CREATE TRIGGER IF NOT EXISTS processamento_extracao_after_insert
        AFTER INSERT ON processamento_extracao
        FOR EACH ROW
        BEGIN
            INSERT INTO log_processamento_extracao (id, alteracao) VALUES (NEW.id, 'inserção');
        END;
    ";
