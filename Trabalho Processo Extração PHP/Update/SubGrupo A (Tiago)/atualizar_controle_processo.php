...
function atualizarDados($id, $temperatura_controle, $tempo_processo, $monitoramento_qualidade, $conn) {
    $sql = "UPDATE controle_processo SET temperatura_controle=?, tempo_processo=?, monitoramento_qualidade=? WHERE id=?";
    $stmt = $conn->prepare($sql);

    // Vincula parâmetros e executa a declaração
    $stmt->bind_param("sssi", $temperatura_controle, $tempo_processo, $monitoramento_qualidade, $id);
